...
Listing~\ref{fig:test_abs} shows our test code whose output is listed in Table~\ref{tbl:test_abs_output}.
...
header file from Listing~\ref{fig:abs2-h} which contains an ACSL specification of \inl{abs_int}.
...
Listing~\ref{fig:abs3a1} shows an implementation of \inl{abs_int}
...
\end{minipage}
\caption{\label{fig:abs3logging1} Calling a logging function from \inl{abs_int}}
\end{listing}
...
To come back to our question about Listing~\ref{fig:abs3a1} it is important
...
one can use an \inl{assigns \\nothing} clause as shown in Listing~\ref{fig:abs3a2}.
...
Listing~\ref{fig:abs3a3} shows how the \inl{assigns} clause of a
