...
int Bitwalker_Poke (unsigned int Startposition, unsigned int Length, uint8_t Bitstream[], unsigned int BitstreamSizeInBytes, uint64_t Value)
{
    ...
    int i;
        ...
        /*@
             loop invariant  Startposition + Length  > i;
             loop invariant i >= Startposition -1;
             loop assigns i, Value, Bitstream[Startposition..(Startposition + Length -1)];
             loop variant i-(Startposition+1);
         */
