...
within \lstinline{bytevalue}, its is discussed in
...
\item each possible bit position in the array to fit into a
	\lstinline{uint32_t} (``\lstinline{size}''), and
...
\item that the value of \lstinline{flag}\footnote{
		...
		\lstinline{SetBit8}, cf. Appendix{subsec:low-level 8}.
	}
...
no memory may be modified (\lstinline{assigns}), and the result is
non-zero if, and only
...
the return value coincides with \lstinline{value}, except possibly
at \lstinline{pos}
...
in Appendix~\ref{subsec:low-level 8} for the 8 bit version; see
in particular
...
%
It was needed to enable the provers to verify code that uses
\lstinline{SetBit64}.
...
\fxfatal{Is there any documentation available? What about negative numbers?}
%
...
\end{lstlisting}
\end{minipage}
\caption{\label{lst:BitTest}The Frama-C library predicate \lstinline{BitTest}}
\end{listing}
