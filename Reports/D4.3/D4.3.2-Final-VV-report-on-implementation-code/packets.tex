...
the ones in Secion~\ref{sec:adhesionfactor-decodebit}.
...
\begin{itemize}
\item
\inl{valid_stream} is only met if \inl{Writeable(stream)} applies.
The predicate \inl{Writeable(Bitstream*)} requires that the
...
one successfull case and two error cases.

\inl{normal_case} describes the behavior of a successfull
...
of the bit stream will be written.
...
bits before the old \inl{stream->bitpos} and the
bits after the new \inl{stream->bitpos}
remain unchanged.
The definition of \inl{Unchanged(Bitstream*, integer, integer)}
is shown in Section~\ref{sec:bitstream}.

\inl{values_too_big} describes the scenario in which
the value of at least one member of \inl{*p} is bigger
than the specified bitsize for that member
of \inl{AdhesionFactor} allows.
...
to false and the \inl{Normal(stream, MaxBitSize(p))}
...
behaviors by the evaulation of the predicate
