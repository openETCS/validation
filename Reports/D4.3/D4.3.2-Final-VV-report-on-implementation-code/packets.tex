...
are defined, each by a \lstinline{struct} declaraion in \lstinline{C}.
We examplify our discussion on the alphabetically first packet,
...
In order to cope with the similarity of specification, implemetation,
and verification tasks for all packets, we have chosen to automatically 
generate \fxfatal{Was genau? struct declaraion, contract, ...} from the
...
\inl{ensures} and \inl{assigns} clauses describe the behavior of the function.

The \inl{assigns} clauses in the contract's body describe the side effects
of the function. If the function contract is split into multiple
behaviors, like here, common \inl{assigns} clauses, which contain the union
of the behavior's \inl{assigns} clauses, are needed outside of the behaviors.
...
The \inl{Readable(Bitstream*)} predicate is described in Chapter~\ref{sec:bitstream}.
\item
\inl{stream_invariant} is only met if \inl{*stream}
is long enough to read an object of the length of \inl{AdhesionFactor} from.
The \inl{Invariant(Bitstream*, integer)} predicate is introduced in Chapter~\ref{sec:bitstream}.
\item
...
%\fxfatal{make sure that \inl{Invariant(Bitstream*, integer)} is described in Chapter~\ref{cha:bitstream}}

%\fxfatal{make sure that \inl{Readable(Bitstream*)} is described in Chapter~\ref{cha:bitstream}}

\inl{normal_case} describes the function's behavior if
\inl{*stream} contains enough unread bits to fill all members of \inl{*p}. 
In this case an object of type \inl{AdhesionFactor} is decoded from the stream
