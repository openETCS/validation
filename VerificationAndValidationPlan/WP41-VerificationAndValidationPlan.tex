...
\reportnum{OETCS/WP4/D4.1V00.08}
...
Izaskun de la Torre (SQS), Jan Welte(TU-BS), Frederic Badeau (Systerel), Marielle Petit-Doche (Systerel)}
...
\hline
  &  Name & Role & Date   \\
\hline  
Written by    &  Hardi Hungar & WP4-T4.1 Task Leader  & August 2013\\
\hline
Approved by & -- & -- & \\
\hline
\end{tabular}
...
respect is of course the interface to the safety consideration and the
contribution of V\&V to the safety case.
...
verification of the software/hardware integration. The API In current
view, the API wil play an important role in defining how the end
result is going to be taken up in a product design. 
...
integration. Sketch how this should look like for the openETCS
architecture approach (with SSRS and API). Ideal would be a
description of how a full openETCS EVC software could be taken up by
some manufacturer and brought to life in a product (validation aspect
only, of course). Validation will use tests covering operational
scenarios.  Not-so-classical validation can start earlier when
executable models become available. If a model can be animated to run
an operational scenario (perhaps with some additional
environment/rest-of-system modeling), design defects may get unveiled
before the real validation.
...
\paragraph{TWT}   
  ...
  SysML models. In first investigations the Acceleo tool (Eclipse
  plugin, based on OMG standard) seems to be a promising candidate for
  model-to-text transformation. The approach will be aligned with URO
  and their SystemC model of the braking curves.  
...
\paragraph{URO}
  ...
  will be constructed as well and aligned with TWT's activities on
  SysML $\rightarrow$ SystemC code generation. 
  See also Sct.~\ref{sec:model-based-sim-openETCS}.
