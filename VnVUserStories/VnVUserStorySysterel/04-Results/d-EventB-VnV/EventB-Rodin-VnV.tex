...
free AtelierB provers, an open source SMT plug-in that supports several
solvers\footnote{supported open source SMT solvers include: verIT, alt-ergo,
  CVC3, Z3} as well as an open source plug-in that connects Rodin to the
...
Plan~\cite{vnvplan}.  In short, in consists of formalizing and proving the
...
A safety analysis identifies additional requirement which guarantee the safety
...
linked to the invariants, inv6, inv7 and inv8.
...
The means used are:
\begin{itemize}
\item Rodin tool (\url{http://www.event-b.org/}), including plug-ins (for
  details
...
Having an abstract, but executable formal model of the implemented functionality
can give interesting insights into the overall functioning of a
...
elements must be considered no a system level analysis and which on the lower
...
\bibitem{vnvplan} Hardi Hungar et.~al, \emph{openETCS Validation\& Verification
...
\bibitem{safetyBrice} Brice Gombault, \emph{Safety analysis of Subset 026,
...
\bibitem{subset-26} UNISIG, \emph{{SUBSET-026} – {S}ystem {R}equirements
