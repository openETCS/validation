...
methodologies to be employed by the verification team. The purpose of
each should be defined and plans for the acquisition, training,
support, and qualification of each shall be described in this
section. 
}

\subsubsection{Reviews and Inspections}
The introduction of reviews in the openETCS project will enable the
adherence to design specifications as well as ensuring that
appropriate coding generation techniques - including relevant coding
standards have been used.  The use of an appropriate review technique
will also help ensure consistency of approach across development teams
...
otherwise have existed in the software. Specialist analysis techniques
exist for predicting the reliability of software.

The only way of verifying any product whether it is a piece of
software, a design specification, test script or anything else is to
carry out some sort of review or inspection process. The introduction
of formal review processes provides us with definite points in time
when we can carry out these essential assessments.
...
and deviations from standards and specifications. Inspections are peer
...
Both Review and inspections methods shall be used in different
...
This section describes our the plans regarding the verification of
...
\item Create a test model in SysML. From the model evaluation
  ...
  available. To cover different aspect of the specification, the ceiling
  speed monitoring model will be also provided.
\item Generate test cases according to the defined interface given by
  DLR. (RT-Tester)
\item Study the automatic generation of the track layouts and speed simulations
\item Set up a test environments for
  \begin{itemize}
    \item Software-in-loop testing with code provided by SCADE
      (Siemens)
    \item Testing within DLR laboratory.
  \end{itemize}
\end{enumerate}
...
radio management communication (see the model-evaluation EA/RT-tester).
...
third level of verification) a particular acitivity is planned. In the
...
acitivities performed by the project partners in relating them to the
