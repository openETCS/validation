...
\fxfatal{Ich w\"urde ja folgende Signatur bevorzugen:}
\begin{lstlisting}[style=acsl-block]
uint64_t  Bitwalker_Peek(unsigned int Startposition, 
                         unsigned int Length,
                         unsigned int BitstreamSizeInBytes,
                         uint8_t Bitstream[BitstreamSizeInBytes]);
\end{lstlisting}
...
\item \inl{Startposition} $\leq$ \inl{UINT_MAX} - \inl{Length}.
      \fxfatal{Besser:  \inl{Startposition} + \inl{Length} $\leq$
      \inl{BitstreamSizeInBytes}*8 $\leq$ \inl{UINT_MAX}}
...
\fxfatal{\inl{Startposition + Length} $\leq$
	\inl{BitstreamSizeInBytes}*8 $\leq$ \inl{UINT_MAX}}
\end{itemize}
...
\begin{itemize}
\item 
If the bit sequence\fxfatal{Welche jetzt? Das Array, in das geschrieben
werden soll, oder die Sequenz, die hineingeschrieben werden soll?} is not valid, then \poke returns~$-1$.
