...
In a first step, we therefore analysed the soure code and 
derived from this an \emph{informal specification}.
This informal specification is to be understood
as a requierements document for the bitwalker functions.

In order to avoid the problem of ``verifying that the code works as implemented''
we put our iformal specification up to review by the developers.
\end{framed}
