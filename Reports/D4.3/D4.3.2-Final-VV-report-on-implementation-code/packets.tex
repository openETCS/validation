...
The \inl{Readable(Bitstream*)} predicate is described in Chapter~\ref{cha:bitstream}.
\item
\inl{stream_invariant} is only met if \inl{*stream}
is long enough to read an object of the length of \inl{AdhesionFactor} from.
The \inl{Invariant(Bitstream*, integer)} predicate is introduced in Chapter~\ref{cha:bitstream}.
\item
